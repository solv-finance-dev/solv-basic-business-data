...
export async function handleOpenFundMarketEvent(param: HandlerParam): Promise<void> {
    const {eventSignature, args, event, transaction} = param;

    let newCarryCollector = '';
    if (eventSignature == 'CreatePool(bytes32,address,address,((address,address,uint256,uint256),(uint16,address,uint64),(address,address,address),(uint256,uint256,uint256,uint64,uint64),address,address,address,uint64,bool,uint256))') {
    ...
    } else if (eventSignature == 'UpdatePoolInfo(bytes32,uint16,address,uint256,uint256,address,address)') {
