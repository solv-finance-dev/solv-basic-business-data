...
function addBigInt(a: string | undefined | null, b: string | undefined | null): string {
    ...
    try {
        ...
        console.warn('BizRedeemSlotInfoHandler: BigInt addition failed', {
            a: aValue,
            b: bValue,
...
function subBigInt(a: string | undefined | null, b: string | undefined | null): string {
    ...
    try {
        ...
        console.warn('BizRedeemSlotInfoHandler: BigInt subtraction failed', {
            a: aValue,
            b: bValue,
            error: error instanceof Error ? error.message : String(error),
        });
...
async function getContractType(
	...
	try {
		...
		console.warn('BizRedeemSlotInfoHandler: Failed to get ContractInfo', {
			chainId,
			contractAddress,
			error: error instanceof Error ? error.message : String(error),
		});
...
function decodeSlotInfoFromBytes(
	...
	try {
		...
		console.warn('BizRedeemSlotInfoHandler: Failed to decode slotInfo bytes', {
			contractType,
...
function extractFieldsFromSlotURI(slotURI: string): {
	...
	try {
		...
		if (!slotURI || !slotURI.startsWith('data:application/json;base64,')) {
			console.warn('BizRedeemSlotInfoHandler: slotURI format invalid', {
				hasSlotURI: !!slotURI,
		...
		try {
			...
			console.warn('BizRedeemSlotInfoHandler: Failed to decode base64', {
				error: error instanceof Error ? error.message : String(error),
		...
		console.debug('BizRedeemSlotInfoHandler: Decoded JSON string', {
		...
		try {
			...
			console.warn('BizRedeemSlotInfoHandler: Failed to parse JSON', {
				error: error instanceof Error ? error.message : String(error),
				jsonStringPrefix: jsonString.substring(0, 200),
				jsonStringLength: jsonString.length,
			});
		...
		if (!parsed.properties || !Array.isArray(parsed.properties)) {
			console.warn('BizRedeemSlotInfoHandler: slotURI missing properties array', {
		...
		if (Object.keys(result).length > 0) {
			console.log('BizRedeemSlotInfoHandler: Extracted from slotURI', {
		...
		console.warn('BizRedeemSlotInfoHandler: Failed to parse slotURI', {
			error: error instanceof Error ? error.message : String(error),
...
async function getSlotURISafe(
	...
	try {
		...
		console.warn('BizRedeemSlotInfoHandler: Failed to get slotURI', {
			chainId,
			contractAddress,
			slot,
			error: error instanceof Error ? error.message : String(error),
...
async function getCurrencySymbol(
	...
	try {
		...
		console.warn('BizRedeemSlotInfoHandler: Failed to get CurrencyInfo', {
			chainId,
			currencyAddress,
			error: error instanceof Error ? error.message : String(error),
		});
...
async function getRedeemSlotInfo(
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: Failed to get RedeemSlotInfo', {
            chainId,
            contractAddress,
            slot,
            error: error instanceof Error ? error.message : String(error),
...
async function handleCreateSlot(
    ...
    if (!slot) {
        console.warn('BizRedeemSlotInfoHandler: CreateSlot missing slot', {
    ...
    if (existing) {
        console.log('BizRedeemSlotInfoHandler: CreateSlot record already exists', {
            contractAddress,
            slot,
            eventId: event.eventId,
        });
	...
	if ((!poolId || !currencyAddress || nav === undefined || nav === null || !createTime) && typeof slotInfo === 'string' && contractType) {
		...
		if (slotInfo.startsWith('0x')) {
			...
			// 	console.debug('BizRedeemSlotInfoHandler: Extracted fields from slotInfo bytes', {
			// 		eventId: event.eventId,
			// 		slot,
			// 		contractType,
			// 		extractedFields: {
	...
	if (!poolId || !currencyAddress || nav === undefined || nav === null || !createTime) {
		console.warn('BizRedeemSlotInfoHandler: CreateSlot missing required fields', {
			eventId: event.eventId,
			slot,
			hasPoolId: !!poolId,
    ...
    try {
        ...
        // console.debug('BizRedeemSlotInfoHandler: CreateSlot success', {
        //     contractAddress,
        //     slot,
        //     poolId,
        //     eventId: event.eventId,
        ...
        console.error('BizRedeemSlotInfoHandler: CreateSlot failed', {
            contractAddress,
            slot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function handleMintValue(
    ...
    if (!slot || !value) {
        console.warn('BizRedeemSlotInfoHandler: MintValue missing required fields', {
            eventId: event.eventId,
            hasSlot: !!slot,
            hasValue: !!value,
        });
    ...
    if (!redeemSlotInfo) {
        console.warn('BizRedeemSlotInfoHandler: MintValue redeemSlotInfo not found', {
            contractAddress,
            slot,
            eventId: event.eventId,
        });
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: MintValue failed', {
            contractAddress,
            slot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function handleClaim(
    ...
    if (!tokenId || !claimValue) {
        console.warn('BizRedeemSlotInfoHandler: Claim missing required fields', {
            eventId: event.eventId,
            hasTokenId: !!tokenId,
    ...
    if (!slot) {
        console.warn('BizRedeemSlotInfoHandler: Claim tokenInfo not found or missing slot', {
            contractAddress,
            tokenId,
            eventId: event.eventId,
        });
    ...
    const redeemSlotInfo = await getRedeemSlotInfo(event.chainId, contractAddress, slot, transaction);
    if (!redeemSlotInfo) {
        console.warn('BizRedeemSlotInfoHandler: Claim redeemSlotInfo not found', {
            contractAddress,
            slot,
            eventId: event.eventId,
        });
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: Claim failed', {
            contractAddress,
            slot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function handleRepay(
    ...
    if (!slot || !repayCurrencyAmount) {
        console.warn('BizRedeemSlotInfoHandler: Repay missing required fields', {
            eventId: event.eventId,
            hasSlot: !!slot,
    ...
    const redeemSlotInfo = await getRedeemSlotInfo(event.chainId, contractAddress, slot, transaction);
    if (!redeemSlotInfo) {
        console.warn('BizRedeemSlotInfoHandler: Repay redeemSlotInfo not found', {
            contractAddress,
            slot,
            eventId: event.eventId,
        });
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: Repay failed', {
            contractAddress,
            slot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function handleBurnValue(
    ...
    if (!tokenId || !burnValue) {
        console.warn('BizRedeemSlotInfoHandler: BurnValue missing required fields', {
            eventId: event.eventId,
            hasTokenId: !!tokenId,
    ...
    const slot = await getSlotFromTokenId(event.chainId, contractAddress, tokenId, transaction);
    if (!slot) {
        console.warn('BizRedeemSlotInfoHandler: BurnValue tokenInfo not found or missing slot', {
            contractAddress,
            tokenId,
            eventId: event.eventId,
        });
    ...
    const redeemSlotInfo = await getRedeemSlotInfo(event.chainId, contractAddress, slot, transaction);
    if (!redeemSlotInfo) {
        console.warn('BizRedeemSlotInfoHandler: BurnValue redeemSlotInfo not found', {
            contractAddress,
            slot,
            eventId: event.eventId,
        });
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: BurnValue failed', {
            contractAddress,
            slot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function getOpenFundRedemption(
    ...
    try {
        ...
        console.warn('BizRedeemSlotInfoHandler: Failed to get PoolOrderInfo', {
            chainId,
            poolId,
            error: error instanceof Error ? error.message : String(error),
        });
...
async function getRedeemSlotInfoByPoolId(
    ...
    if (!openFundRedemption) {
        console.warn('BizRedeemSlotInfoHandler: PoolOrder not found or missing openFundRedemption', {
            poolId,
            eventId,
        });
        return null;
    ...
    if (!redeemSlotInfo) {
        console.warn('BizRedeemSlotInfoHandler: RedeemSlotInfo not found', {
...
async function handleSetRedeemNav(
    ...
    if (!poolId || !redeemSlot || !nav) {
        console.warn('BizRedeemSlotInfoHandler: SetRedeemNav missing required fields', {
            eventId: event.eventId,
            hasPoolId: !!poolId,
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: SetRedeemNav failed', {
            poolId,
            slot: redeemSlot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
async function handleCloseRedeemSlot(
    ...
    if (!poolId || !previousRedeemSlot) {
        console.warn('BizRedeemSlotInfoHandler: CloseRedeemSlot missing required fields', {
            eventId: event.eventId,
            hasPoolId: !!poolId,
    ...
    try {
        ...
        console.error('BizRedeemSlotInfoHandler: CloseRedeemSlot failed', {
            poolId,
            slot: previousRedeemSlot,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
...
export async function handleOpenFundMarketEvent(param: HandlerParam): Promise<void> {
    ...
    try {
        ...
        switch (eventFunc) {
            ...
            default:
                console.warn('BizRedeemSlotInfoHandler: unhandled OpenFundMarket event signature', {
                    eventFunc,
                    eventId: event.eventId,
                });
        }
    } catch (error) {
        console.error('BizRedeemSlotInfoHandler: handleOpenFundMarketEvent failed', {
            eventFunc,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
        });
...
export async function handleRedeemShareDelegateEvent(param: HandlerParam): Promise<void> {
    ...
    try {
        ...
        switch (eventFunc) {
            ...
            default:
                console.warn('BizRedeemSlotInfoHandler: unhandled event signature', {
                    eventFunc,
                    eventId: event.eventId,
                });
        }
    } catch (error) {
        console.error('BizRedeemSlotInfoHandler: handleRedeemShareDelegateEvent failed', {
            eventFunc,
            eventId: event.eventId,
            error: error instanceof Error ? error.message : String(error),
        });
