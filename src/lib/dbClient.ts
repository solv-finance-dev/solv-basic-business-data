...
import {initRawSequelize, initBusinessSequelize} from './db';
...
// 获取业务数据库（目前对应的是raw_那个库，未来raw这些表会随着业务系统下线而下掉，后续功能迁移到下面 Business 库）
export async function getRawSequelize(): Promise<Sequelize> {
    const now = Date.now();
    ...
    if (!cachedBasicSequelize || (lastBasicInitTime && now - lastBasicInitTime > CACHE_BASIC_EXPIRY_TIME)) {
        ...
        cachedBasicSequelize = await initRawSequelize();
